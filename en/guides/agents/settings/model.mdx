...
description: "How to choose the provider and LLM model for each agent in Rovax.io, capability comparison, credit cost, and selection best practices."
---

The language model defines the intelligence, speed, and cost of each agent execution. Rovax.io supports four providers with access to multiple models — each agent can use a different model.
...
Rovax.io offers two modes of model access: using the platform's credits (access via Rovax) or connecting your own API key from an external provider.

### Option 1 — Using Rovax credits

Each model consumes a fixed number of credits per execution. You do not need to create accounts with providers — Rovax manages authentication and billing.
