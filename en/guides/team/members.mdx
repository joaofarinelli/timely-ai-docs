...
Member management lets you add collaborators to the Rovax.io workspace, assign roles with specific permissions, and control who has access to each resource. All management is done in **Settings > Team**. Only `owner` and `manager` have access to this subsection.
...
<Steps>
  ...
  <Step title="Enter the email address">
    Type the collaborator's email address. They do not need to have an existing Rovax.io account.
  </Step>
...
Rovax.io uses a hierarchical role system. Each role inherits a fixed set of permissions that cannot be customized individually:
