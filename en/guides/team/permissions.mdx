---
title: Permissions
description: Complete permissions matrix by role and how the access control system works in Rovax.io.
---

Rovax.io's access control is role-based (RBAC). Each member has an assigned role that defines exactly what they can see and do within the platform. Permissions are enforced both in the interface and in API calls.
...
When using the Rovax.io API with an API key, endpoint permissions are determined by the scopes configured on the key, not by the role of the user who created it. See [API Keys](/en/guides/team/api-keys) for details on available scopes.
