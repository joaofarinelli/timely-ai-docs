...
- An MCP server developed once works with any compatible client — the Rovax.io agent, Claude Desktop, code editors with AI support, and others.
...
> The Rovax.io agent acts as an MCP client: it maintains a session with each configured server, discovers the available tools, and registers them in the model context. When the LLM decides to trigger a server tool, the client executes the call via SSE or HTTP, receives the result, and injects it into the context before the next iteration.
