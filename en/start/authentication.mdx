...
Authentication with the Rovax.io API is done via an **API key** sent on every request in the `x-api-key` header. No OAuth, no sessions — just the key.
...
For environment variables, always store the key as `ROVAX_API_KEY` and reference it via `process.env` (Node), `os.environ` (Python), or `os.Getenv` (Go). Never hardcode it.

## Creating an API key

1. Go to [app.rovax.io](https://app.rovax.io)
