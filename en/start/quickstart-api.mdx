...
title: "First request"
icon: "paper-plane"
description: "Generate your API key and make your first call to the Rovax.io API in under 5 minutes"
---
...
<Steps>
  <Step title="Generate your API key">
    Go to the [Dashboard](https://app.rovax.io) → **Settings → API Keys → New key**.
  ...
  <Step title="Test the health check">
    ...
    ```
    ```js Node.js
    const res = await fetch("https://api.rovax.io/v1/health", {
      headers: { "x-api-key": process.env.ROVAX_KEY }
    });
    ...
    r = httpx.get(
        "https://api.rovax.io/v1/health",
        headers={"x-api-key": os.environ["ROVAX_KEY"]}
    )
    print(r.json())
    ```
    </CodeGroup>
  ...
  <Step title="List your agents">
    ...
    <CodeGroup>
    ```bash cURL
    curl "https://api.rovax.io/v1/agents?page=1&per_page=10" \
      -H "x-api-key: YOUR_KEY"
    ```
    ```js Node.js
    const res = await fetch(
      "https://api.rovax.io/v1/agents?page=1&per_page=10",
      { headers: { "x-api-key": process.env.ROVAX_KEY } }
    );
  ...
  <Step title="Send a message via agent">
    ...
    <CodeGroup>
    ```bash cURL
    curl -X POST https://api.rovax.io/v1/messages/send \
      -H "x-api-key: YOUR_KEY" \
    ...
    ```
    ```js Node.js
    const res = await fetch("https://api.rovax.io/v1/messages/send", {
      method: "POST",
      headers: {
        "x-api-key": process.env.ROVAX_KEY,
    ...
    r = httpx.post(
        "https://api.rovax.io/v1/messages/send",
        headers={
            "x-api-key": os.environ["ROVAX_KEY"],
            "Content-Type": "application/json"
        },
        json={
            "agent_id": "AGENT_ID",
