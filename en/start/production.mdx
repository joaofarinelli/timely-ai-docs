...
<AccordionGroup>
  ...
  <Accordion title="Webhook secret configured and validated">
    Confirm that the handler validates the `X-Rovax-Signature` header before processing any event. Test with a tampered payload — it should return `401`.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Production URL and API key are correct">
    Confirm the code points to `https://api.rovax.io/v1` and that the `x-api-key` belongs to the production workspace — not a test workspace. In CI/CD, use separate environment variables per stage.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Endpoint publicly accessible">
    ...
    ```bash
    curl -X POST https://yoursite.com/webhook/rovax \
  ...
  <Accordion title="Idempotency implemented">
    The handler uses `X-Rovax-Event-Id` to deduplicate events delivered more than once during retries.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Support contact saved">
    Save **contato@rovax.io** in your incident contacts list. For critical issues, include "URGENT" in the subject line.
  </Accordion>
</AccordionGroup>
...
<Note>
  Checked everything? Go ahead. For any last-minute questions, reach us at **contato@rovax.io**.
</Note>
