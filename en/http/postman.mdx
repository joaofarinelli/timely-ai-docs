---
title: "Postman"
description: "Import the Rovax.io collection into Postman, configure the environment, and run your first request in under 5 minutes."
---

Postman is the most widely used HTTP request tool in development teams. The Rovax.io collection covers all API endpoints with ready-to-use payload examples and response handling.

## Download the collection

Download the collection JSON file:

<Card title="rovax.postman_collection.json" icon="download" href="/http/collections/rovax.postman_collection.json">
...
<Steps>
  ...
  <Step title="Import the file">
    Click **Import** in the upper-left corner. Drag the `rovax.postman_collection.json` file into the drop area, or click **Select Files** and select the file.

    The **Rovax.io API** collection will appear in the sidebar under **Collections**.
  </Step>
  <Step title="Create the environment">
    Click the eye icon in the upper-right corner (or go to **Environments → + New Environment**) and create an environment named **Rovax.io - Production** with the following variables:

    | Variable | Initial Value | Current Value |
    |---|---|---|
    | `base_url` | `https://api.rovax.io` | `https://api.rovax.io` |
...
When the API evolves with new endpoints or fields, download the updated JSON file from `/http/collections/rovax.postman_collection.json` and import it again. Postman will ask whether you want to **replace** or **create a copy**. Choose **Replace** to keep your execution history.
...
2. Select the **Rovax.io - Production** environment
3. Click **Run Rovax.io API**
