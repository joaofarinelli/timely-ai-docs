...
When you connect your Instagram profile in Rovax.io, the platform registers a webhook in the Facebook App associated with your Instagram Business account. Meta then sends events from that profile to Rovax, which normalizes them and forwards them to your [workspace webhooks](/en/webhooks/overview).
...
<Info>
  Rovax.io handles all this configuration automatically when you connect Instagram via the dashboard under **Settings → Channels → Instagram**. You only need this documentation if you are integrating directly with the Graph API.
</Info>
...
<CardGroup cols={2}>
  <Card title="Platform webhooks" icon="bell" href="/en/webhooks/overview">
    Normalized events that Rovax.io sends to your server.
  </Card>
