...
Rovax.io calls the `setWebhook` method of the Bot API pointing to its servers:

```
POST https://api.telegram.org/bot{TOKEN}/setWebhook
{
  "url": "https://webhooks.rovax.io/telegram/{CHANNEL_ID}",
...
<Info>
  If you manage the bot directly (without Rovax as an intermediary), never configure two webhooks on the same token — Telegram delivers each update to only one endpoint.
</Info>
...
<Note>
  Rovax.io treats `edited_message` as an update to the original message. Edit history is not stored — only the most recent content is displayed in the conversation.
</Note>
...
```json
{
  ...
  "callback_query": {
    ...
    "message": {
      "message_id": 45,
      "from": { "id": 111111111, "is_bot": true, "first_name": "Rovax Bot" },
      "chat": { "id": 987654321, "type": "private" },
      "date": 1713456950,
...
<CardGroup cols={2}>
  <Card title="Platform webhooks" icon="bell" href="/en/webhooks/overview">
    Normalized events that Rovax.io fires to your server.
  </Card>
