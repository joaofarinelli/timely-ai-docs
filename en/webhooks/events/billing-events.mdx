...
<Tip>
  If you are a Rovax.io partner or reseller managing multiple workspaces, use `trial.expiring` and `trial.expired` to automate conversion follow-ups. The `conversations_used` and `messages_used` fields show customer engagement during the trial — great data for personalizing your outreach.
</Tip>
