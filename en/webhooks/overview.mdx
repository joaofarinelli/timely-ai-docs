...
Webhooks are HTTP `POST` requests that Rovax.io automatically sends to the URL you configure whenever a relevant event occurs in your workspace. Instead of polling the API, you receive data instantly — no latency, no wasted requests.
...
<Steps>
  ...
  <Step title="Rovax.io sends a POST">
    We send a `POST` with the event's JSON payload to your URL, including the `X-Rovax-Signature` header so you can validate authenticity.
  </Step>
...
<Tabs>
  <Tab title="Development">
    ...
    Set `https://abc123.ngrok.io/webhooks/rovax` as the URL in the dashboard. Remember to update it every time you restart ngrok.
  ...
  <Tab title="Production">
    Always use HTTPS with a valid certificate. Rovax.io rejects HTTP URLs in production. Make sure your URL responds in under 10 seconds — move heavy processing to an async queue.
  </Tab>
</Tabs>

## Security: HMAC signature validation

Every delivery includes the `X-Rovax-Signature` header in the format `sha256=HEX`. This value is the HMAC-SHA256 of the request body using the **secret** generated when the webhook was created.
...
If your URL does not respond with `2xx` within 10 seconds, Rovax.io automatically retries with exponential backoff:
...
<Tabs>
  ...
  <Tab title="API">
    ...
    curl -X POST https://api.rovax.io/v1/webhooks \
      ...
      -d '{
        "url": "https://your-server.com/webhooks/rovax",
