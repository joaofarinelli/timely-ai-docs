...
description: "Endpoints to create, update, and manage contacts in the Rovax.io CRM"
---

## Concept

**Contacts** are customer records in the Rovax.io integrated CRM. Every conversation that arrives through any channel (WhatsApp, Instagram, Telegram, widget) is automatically associated with a contact — created based on the channel identifier (phone number, IGSID, Telegram chat_id) or linked to an existing contact by email or phone.
...
Rovax tries to avoid duplicate contacts when automatically creating one during a conversation. Deduplication uses the following priority:

1. Channel identifier (phone for WhatsApp, IGSID for Instagram)
2. Email (if available)
3. Normalized phone number (E.164)

When creating via API, if a contact with the same `phone` or `email` already exists, Rovax returns the existing contact instead of creating a duplicate.
