...
**MCP Servers** (Model Context Protocol) are external servers that expose tools and resources so that Rovax.io agents can perform real-world actions during a conversation. Instead of the agent simply responding with text, it can use MCP tools to query a database, create a calendar event, look up an order in e-commerce, send an email, or any other action the MCP server supports.

The MCP protocol is an open standard — any compatible server can be connected to a Rovax agent. You can use publicly available community servers or develop your own to integrate your company's internal systems.
...
AI agent processes and decides to use a tool
    │
    ▼
Rovax.io calls the MCP Server with the tool parameters
    │
    ▼
...
add server (URL + configuration)
    │
    ▼
connect → Rovax handshakes with the server
    │
    ▼
