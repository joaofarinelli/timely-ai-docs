...
A message in Rovax.io represents any unit of communication exchanged in a conversation: text, image, audio, video, document, or interactive message (buttons, lists). Each message records the sender (contact, AI agent, or human), the source channel, delivery status, and platform metadata (such as the WhatsApp `wamid` or Telegram `message_id`).

## Main Fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | string (UUID) | Unique message ID in Rovax |
