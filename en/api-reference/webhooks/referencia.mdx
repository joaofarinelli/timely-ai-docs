...
Rovax.io **Webhooks** are HTTP `POST` notifications automatically sent to the URL you configure whenever a relevant event occurs in the workspace — a message received, a conversation closed, a contact created, a channel disconnected, and more. Instead of polling the API periodically to check whether something changed, you receive the data as soon as the event occurs.

Each webhook has a **secret** generated at creation time, used to sign deliveries with HMAC-SHA256 in the `X-Rovax-Signature` header. This ensures only Rovax.io can send events to your endpoint.
...
create webhook (URL + events + generated secret)
    │
    ▼
active → Rovax delivers events to the URL
    │
