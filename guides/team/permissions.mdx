---
title: Permissões
description: Matriz completa de permissões por role e como o sistema de controle de acesso funciona na Rovax.io.
---

O controle de acesso da Rovax.io é baseado em roles (RBAC). Cada membro tem uma role atribuída que define exatamente o que ele pode ver e fazer dentro da plataforma. As permissões são aplicadas tanto na interface quanto nas chamadas de API.
...
Ao usar a API da Rovax.io com uma API key, as permissões do endpoint são determinadas pelos escopos configurados na chave, não pela role do usuário que a criou. Veja [API Keys](/guides/team/api-keys) para detalhes sobre escopos disponíveis.
