---
title: "Nó Webhook"
description: "Receba requisições HTTP externas e use o payload como ponto de entrada do seu workflow na Rovax.io."
---

## Como Funciona

O nó Webhook gera um endpoint HTTP dedicado ao seu workflow. Quando um sistema externo — um e-commerce, um formulário, um ERP ou qualquer serviço — envia uma requisição para essa URL, a Rovax valida a chamada e inicia a execução imediatamente.
...
Quando definida como **Bearer Token**, a Rovax valida o header `Authorization` em cada requisição recebida. Requisições que não incluam o header ou que apresentem um token diferente do configurado recebem `401 Unauthorized` automaticamente, sem acionar a execução do workflow.
