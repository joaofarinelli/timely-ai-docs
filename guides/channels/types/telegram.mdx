...
O canal Telegram usa a API de bots do Telegram. Você cria um bot pelo **BotFather** (o bot oficial do Telegram para gerenciar outros bots), copia o token gerado e cola na Rovax.io. A plataforma configura o webhook automaticamente.
...
<Steps>
  <Step title="Crie um bot no BotFather">
    ...
    1. **Nome do bot** — nome amigável que aparece nas conversas (ex: "Suporte Rovax")
    2. **Username do bot** — deve terminar em `bot` (ex: `suporte_rovax_bot`)
  ...
  <Step title="Acesse Canais no dashboard">
    No menu lateral da Rovax.io, acesse **Canais → Adicionar canal** e selecione **Telegram**.
  </Step>

  <Step title="Cole o token e conecte">
    Informe o **token do bot** no campo correspondente. O token precisa estar no formato `número:string` (ex: `123456789:ABCdef...`). A Rovax.io valida o token com a API do Telegram antes de prosseguir.
  ...
  <Step title="Webhook configurado automaticamente">
    Ao confirmar o token válido, a Rovax.io:
  ...
  <Step title="Teste o bot">
    Abra o Telegram, procure pelo username do seu bot (ex: `@suporte_rovax_bot`) e envie uma mensagem. O agente deve responder em até 2 segundos.
  </Step>
</Steps>
...
A Rovax.io valida o header `X-Telegram-Bot-Api-Secret-Token` em cada requisição. Para detalhes sobre o payload e eventos suportados, veja [Webhooks externos → Telegram](/webhooks/external/telegram).
...
- **Rotação de token** — se suspeitar que o token foi comprometido, revogue-o no BotFather com `/revoke` e gere um novo. Atualize na Rovax.io em **Canais → [sua integração Telegram] → Editar**.
...
<AccordionGroup>
  ...
  <Accordion title="Bot não responde após a conexão">
    ...
    O campo `url` deve apontar para `https://[SEU-SUPABASE].supabase.co/functions/v1/telegram-webhook`. Se estiver vazio, desconecte e reconecte o canal na Rovax.io para forçar o `setWebhook`.
  </Accordion>
  ...
  <Accordion title="Como trocar o token sem perder o histórico?">
    Acesse **Canais → [integração Telegram] → Editar**, atualize o token e salve. A Rovax.io chama `setWebhook` com o novo token automaticamente. O histórico de conversas fica preservado no banco de dados da Rovax.io.
  </Accordion>
</AccordionGroup>
