...
O agente da Rovax.io consegue gerenciar o ciclo completo de agendamentos sem intervenção humana: consulta disponibilidade, confirma com o cliente, cria o agendamento, envia lembretes e processa reagendamentos ou cancelamentos quando solicitados.
...
A Rovax.io envia lembretes automáticos para o cliente antes do horário agendado. As configurações padrão são:
...
Todos os agendamentos ficam registrados na seção **Agendamentos** do painel da Rovax.io. Você pode filtrar por agente, calendário, data e status (`confirmed`, `cancelled`, `completed`, `no_show`).
