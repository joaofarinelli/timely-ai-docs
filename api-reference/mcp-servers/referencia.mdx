...
**MCP Servers** (Model Context Protocol) são servidores externos que expõem ferramentas e recursos para que os agentes da Rovax.io possam executar ações no mundo real durante uma conversa. Em vez de o agente apenas responder com texto, ele pode usar ferramentas do MCP para consultar um banco de dados, criar um evento no calendário, buscar um pedido no e-commerce, enviar um email ou qualquer outra ação que o servidor MCP suporte.

O protocolo MCP é um padrão aberto — qualquer servidor compatível pode ser conectado a um agente Rovax. Você pode usar servidores públicos disponíveis na comunidade ou desenvolver o seu próprio para integrar sistemas internos da sua empresa.
...
agente IA processa e decide usar uma tool
    │
    ▼
Rovax.io chama o MCP Server com os parâmetros da tool
    │
    ▼
...
adicionar servidor (URL + configuração)
    │
    ▼
conectar → Rovax faz handshake com o servidor
    │
    ▼
