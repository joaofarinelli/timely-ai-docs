...
Os **Webhooks** da Rovax.io são notificações HTTP `POST` enviadas automaticamente para a URL que você configurar sempre que um evento relevante acontece no workspace — uma mensagem recebida, uma conversa fechada, um contato criado, um canal desconectado, entre outros. Em vez de consultar a API periodicamente para verificar se algo mudou, você recebe o dado assim que o evento ocorre.

Cada webhook tem um **secret** gerado no momento da criação, usado para assinar as entregas com HMAC-SHA256 no header `X-Rovax-Signature`. Isso garante que apenas a Rovax.io pode enviar eventos para o seu endpoint.
...
criar webhook (URL + eventos + secret gerado)
    │
    ▼
active → Rovax entrega eventos para a URL
    │
