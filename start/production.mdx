...
<AccordionGroup>
  ...
  <Accordion title="Webhook secret configurado e validado">
    Confirme que o handler valida o header `X-Rovax-Signature` antes de processar qualquer evento. Teste com um payload adulterado — deve retornar `401`.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="URL de produção e API key corretas">
    Confirme que o código aponta para `https://api.rovax.io/v1` e que a `x-api-key` é do workspace de produção — não de um workspace de testes. Em CI/CD, use variáveis de ambiente distintas por stage.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Endpoint acessível publicamente">
    ...
    ```bash
    curl -X POST https://seusite.com.br/webhook/rovax \
  ...
  <Accordion title="Idempotência implementada">
    O handler usa `X-Rovax-Event-Id` para deduplicar eventos entregues mais de uma vez durante retentativas.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Contato de suporte salvo">
    Guarde **contato@rovax.io** na sua lista de contatos de incidente. Em casos críticos, mencione "URGENTE" no assunto.
  </Accordion>
</AccordionGroup>
...
<Note>
  Passou em tudo? Va em frente. Qualquer duvida de ultima hora, estamos em **contato@rovax.io**.
</Note>
